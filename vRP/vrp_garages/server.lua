...
--vRPsh = Tunnel.getInterface("vrp_showroom", "vRP_garages")
...
local cfgSR = module("vrp_showroom","cfg/config")
local vehiclesList = cfgSR.showgarage

--SQL--
MySQL.createCommand("vRP/ply_get_vehicles","SELECT * FROM vrp_user_vehicles WHERE user_id = @user_id")
MySQL.createCommand("vRP/ply_del_vehicles", "DELETE FROM vrp_user_vehicles WHERE user_id = @user_id and vehicle = @vehicle")
...
RegisterServerEvent('ply_garages:SellVeh')
AddEventHandler('ply_garages:SellVeh', function(vehicle)
    local user_id = vRP.getUserId({source})
    local player = vRP.getUserSource({user_id})

    for v=1, #vehiclesList.vehicles do
      if vehiclesList.vehicles[v].model == vehicle then
        local oriprice = vehiclesList.vehicles[v].costs
        local sellprice = math.ceil(oriprice*0.75) -- 75% de perca
        vRP.giveMoney({user_id, sellprice})
        MySQL.query("vRP/ply_del_vehicles", {user_id = user_id, vehicle = vehicle})
        vRPclient.notify(player,{"~g~Veículo "..vehicle.." foi vendido."})
        vRPclient.notify(player,{"~g~Recebeu R$ "..sellprice})
        vRP.closeMenu({player})
      else
        vRPclient.notify(player,{"~g~Este veículo não pode ser vendido no momento."})
      end
    end
  end)
